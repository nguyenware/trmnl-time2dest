...
// TomTom repeats the road number in its location names, e.g.
// "Bridge Way/Fremont Way (WA-99)"; the road is already shown on its own.
export function stripRoad(place, road) {
  if (!place || !road) return place ?? '';
  const escaped = road.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return place.replace(new RegExp(`\\s*\\(${escaped}\\)`, 'gi'), '').trim();
}

function incidentView(incident, index, route) {
  const aheadMi = miles(incident.alongM);
  const from = stripRoad(incident.from, incident.road);
  const to = stripRoad(incident.to, incident.road);
  const where = from && to && from !== to ? `${from} → ${to}` : from || to;
